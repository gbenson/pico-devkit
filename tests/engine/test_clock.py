...
def test_init_defaults():
    c = Clock()
    assert c.max_framerate == approx(60)
    assert c.min_interval == approx(1/60)
    assert c.min_interval_us == approx(1e6/60)


def test_init_with_limit():
    c = Clock(23 * PI)
    assert c.max_framerate == approx(72.2566)
    assert c.min_interval == approx(0.01383956)
    assert c.min_interval_us == approx(13839.56)


def test_init_without_limit():
    c = Clock(None)
    assert c.max_framerate is None
...
def test_setters():
    ...
    assert c.max_framerate == approx(59.9964)
    assert c.max_framerate != approx(60)
    ...
    assert c.max_framerate == approx(53.3301)


@patch("target.engine.sleep_us")
@pytest.mark.parametrize(
    "attr",
    ("max_framerate", "min_interval", "min_interval_us"),
)
@pytest.mark.parametrize(
    "value",
    (0, -1, -1.0, 0.0, None, False),
)
def test_clear_max_framerate(sleep_us: Mock, attr: str, value: Any):
    c = Clock(50)
...
def test_no_unnecessary_sleeping(sleep_us: Mock):
    c = Clock()
    c.min_interval_us = 1
    assert c.max_framerate == approx(1_000_000)
